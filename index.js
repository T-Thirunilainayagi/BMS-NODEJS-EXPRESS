const state = {
  ...
  const htmlTaskContent = ({
    ...
    <div class="card shadow-sm task__card">
      ...
      ${
        url &&
        `<img  src=${url} alt="Card image cap" class="card-img-top mb-3 rounded-lg" width="300" height="300" >`
      }
  ...
  const htmlModalContent = ({ id, title, description, url }) => {
    ...
    return ` <div id=${id}>
      ...
      <p class="lead">
      <a class="prodlink" href= ${description} target="_blank"  >${description}</a>
      </p></div>`;
  };
  ...
  const deleteTask = (e) => {
    ...
    updateLocalStorage();
    if (type === "button")
      return e.target.parentNode.removeChild(
        e.target.parentNode
      );
      // 4 3    5 4 pns in above and below
